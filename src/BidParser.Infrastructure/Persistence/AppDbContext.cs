...
/// <summary>
/// EF Core context (SQL Server, pooled). Owns the five tables — users, parse_jobs, parse_metrics,
/// failed_parse_jobs, dell_api_settings — with their snake_case column mappings, indexes and delete behaviours configured in
/// <see cref="OnModelCreating"/>. All entity timestamps are set centrally by <see cref="StampTimestamps"/>
/// on every save, so entities must never initialise CreatedAt/UpdatedAt themselves.
/// </summary>
public sealed class AppDbContext : DbContext
{
    ...
    public DbSet<DellApiSettings> DellApiSettings => Set<DellApiSettings>();
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<ParseJob>(entity =>
        {
            ...
            entity.Property(job => job.BidNumber).HasColumnName("bid_number").UseCollation("SQL_Latin1_General_CP1_CI_AS").HasMaxLength(128);
            entity.Property(job => job.BidRevision).HasColumnName("bid_revision").UseCollation("SQL_Latin1_General_CP1_CI_AS").HasMaxLength(32);
            ...
            entity.Property(job => job.SplitBySolutionId).HasColumnName("split_by_solution_id").IsRequired();
            entity.Property(job => job.ImportType).HasColumnName("import_type").HasMaxLength(16)
                .HasConversion(
                    v => v == null ? null : v.Value.ToString().ToLowerInvariant(),
                    v => v == null ? (ImportType?)null : Enum.Parse<ImportType>(v, ignoreCase: true));
        ...
        modelBuilder.Entity<ParseMetric>(entity =>
        {
            ...
            entity.Property(m => m.ImportType).HasColumnName("import_type").HasMaxLength(16)
                .HasConversion(
                    v => v == null ? null : v.Value.ToString().ToLowerInvariant(),
                    v => v == null ? (ImportType?)null : Enum.Parse<ImportType>(v, ignoreCase: true));
        ...
        modelBuilder.Entity<FailedParseJob>(entity =>
        {
            ...
            entity.Property(f => f.ImportType).HasColumnName("import_type").HasMaxLength(16)
                .HasConversion(
                    v => v == null ? null : v.Value.ToString().ToLowerInvariant(),
                    v => v == null ? (ImportType?)null : Enum.Parse<ImportType>(v, ignoreCase: true));
        ...
        modelBuilder.Entity<DellApiSettings>(entity =>
        {
            entity.ToTable("dell_api_settings", table =>
                table.HasCheckConstraint("ck_dell_api_settings_singleton", "[id] = 1"));
            entity.HasKey(settings => settings.Id);
            entity.Property(settings => settings.Id).HasColumnName("id").ValueGeneratedNever();
            entity.Property(settings => settings.TokenUrl).HasColumnName("token_url").HasMaxLength(512).IsRequired();
            entity.Property(settings => settings.ClientId).HasColumnName("client_id").HasMaxLength(256).IsRequired();
            entity.Property(settings => settings.ClientSecretProtected).HasColumnName("client_secret_protected").IsRequired();
            entity.Property(settings => settings.QuoteUrlTemplate).HasColumnName("quote_url_template").HasMaxLength(1024).IsRequired();
            entity.Property(settings => settings.DefaultLocale).HasColumnName("default_locale").HasMaxLength(16).IsRequired();
            entity.Property(settings => settings.ClientIdHeader).HasColumnName("client_id_header").HasMaxLength(128).IsRequired();
            entity.Property(settings => settings.ApiVersion).HasColumnName("api_version").HasMaxLength(16).IsRequired();
            entity.Property(settings => settings.UseBasicAuthForToken).HasColumnName("use_basic_auth_for_token").IsRequired();
            entity.Property(settings => settings.CreatedAt).HasColumnName("created_at").IsRequired();
            entity.Property(settings => settings.UpdatedAt).HasColumnName("updated_at").IsRequired();
        });

        modelBuilder.Entity<RuntimeConfig>(entity =>
        {
            entity.ToTable("runtime_configs");
            entity.HasKey(config => config.Key);
            entity.Property(config => config.Key).HasColumnName("key").HasMaxLength(64);
            entity.Property(config => config.JsonPayload).HasColumnName("json_payload").HasColumnType("nvarchar(max)").IsRequired();
            entity.Property(config => config.CreatedAt).HasColumnName("created_at").IsRequired();
            entity.Property(config => config.UpdatedAt).HasColumnName("updated_at").IsRequired();
        });
    }

    /// <summary>
    /// The single place CreatedAt/UpdatedAt are assigned: on save it stamps CreatedAt for newly-added
    /// rows and bumps User.UpdatedAt on add/modify. Do NOT add timestamp initialisers on entities.
    /// </summary>
    private void StampTimestamps()
    {
        ...
        foreach (var entry in ChangeTracker.Entries<DellApiSettings>())
        {
            if (entry.State == EntityState.Added)
            {
                if (entry.Entity.CreatedAt == default) entry.Entity.CreatedAt = now;
        ...
        foreach (var entry in ChangeTracker.Entries<RuntimeConfig>())
        {
            if (entry.State == EntityState.Added)
            {
                if (entry.Entity.CreatedAt == default) entry.Entity.CreatedAt = now;
