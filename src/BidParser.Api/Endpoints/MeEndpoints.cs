...
/// <summary>
/// Current-user endpoints: GET /api/me (the signed-in user's public profile) and PATCH /api/me/settings
/// (update saved preferences — CSRF-guarded, active users only). The vendor allowlist is built from the
/// <see cref="Vendors"/> constants so a new vendor can't drift out of it.
/// </summary>
public static class MeEndpoints
{
    // Built from the Vendors constants so a new vendor can't silently drift out of
    // the allowlist. Must cover every vendor a parser is registered under, or this
    // endpoint rejects a default_vendor that ParseService persists on a successful parse.
    ...
    private static async Task<IResult> MeAsync(HttpContext context, AppDbContext db, CancellationToken ct)
    {
        ...
        return user is null
            ? Results.Json(new ApiError("notAuthenticated"), statusCode: StatusCodes.Status401Unauthorized)
        ...
        CancellationToken ct)
    {
        ...
        if (user is null)
        {
            return Results.Json(new ApiError("notAuthenticated"), statusCode: StatusCodes.Status401Unauthorized);
        }
