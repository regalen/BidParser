...
/// <summary>
/// Authentication endpoints under /api/auth (CSRF-guarded): POST /login (anonymous, rate-limited),
/// /logout and /change-password (logged-in). Login is written to cost one BCrypt verification whether or
/// not the username exists, so response timing doesn't leak which usernames are valid.
/// </summary>
