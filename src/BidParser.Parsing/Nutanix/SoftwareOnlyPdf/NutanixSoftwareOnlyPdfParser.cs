...
/// <summary>
/// Parses Nutanix "Software Only" quote PDFs. Anchor-based: locates the "Product Code" table in the
/// positioned word stream, buckets rows/columns by coordinates, and reads the quote TOTAL. Foreign
/// currency (USD) → "Foreign Uplift" output; the term is emitted as a "{n} Months" comment.
/// See docs/nutanix_software_only_pdf.md.
/// </summary>
public sealed partial class NutanixSoftwareOnlyPdfParser : IParser
{
    ...
    /// <summary>
    /// Wrong-file-type hint (not routing): 0.8 when a "Product Code" table exists with neither the
    /// Quote D banner (that's Hardware Only) nor a "Serial" column (that's Renewal).
    /// </summary>
    ...
    public ParseResult Parse(string path)
    {
        ...
        var bid = BidMetadataCleaner.CleanRevisionless(ExtractBidNumber(words, header));
    ...
    private static LineItem BuildItem(CurrentItem item)
    {
        ...
        return new LineItem
        {
            Vpn = NutanixVpnNormalizer.Normalize(TextCleaner.JoinUnspaced(item.CodeParts)),
