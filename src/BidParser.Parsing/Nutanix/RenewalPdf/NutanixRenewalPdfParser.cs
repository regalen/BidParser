...
/// <summary>
/// Parses Nutanix "Renewal" quote PDFs. Anchor-based on the line-item table's "Serial" column (unique
/// to Renewal among the Nutanix PDF formats). Fuses split "USD 1,234" amounts before bucketing rows by
/// coordinates. Foreign-currency → "Foreign Uplift" output. See docs/nutanix_renewal_pdf.md.
/// </summary>
public sealed partial class NutanixRenewalPdfParser : IParser
{
    ...
    /// <summary>
    /// Wrong-file-type hint (not routing): 0.9 for a "Serial" column on the line-item table with no
    /// Quote D banner (Hardware Only). Software Only has a Product Code table but no Serial column.
    /// </summary>
    ...
    public ParseResult Parse(string path)
    {
        ...
        var bid = BidMetadataCleaner.CleanRevisionless(ExtractBidNumber(fused, header));
    ...
    /// <summary>Reads the "Quote: XQ-…" document header from the words preceding the table header.</summary>
    private static string ExtractBidNumber(IReadOnlyList<PdfWord> words, PdfWord header)
    {
        var text = PdfTableHelpers.WordStreamText(words.TakeWhile(word => !ReferenceEquals(word, header)));
        return QuoteField().Match(text).Groups[1].Value;
    }

    [GeneratedRegex(@"Quote\s*:\s*(XQ-\d+)", RegexOptions.IgnoreCase)]
    private static partial Regex QuoteField();
    ...
    private static LineItem BuildItem(CurrentItem item)
    {
        ...
        return new LineItem
        {
            Vpn = NutanixVpnNormalizer.Normalize(TextCleaner.JoinUnspaced(item.ProductCodeParts)),
