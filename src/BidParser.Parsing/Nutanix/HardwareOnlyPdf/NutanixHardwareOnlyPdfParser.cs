...
/// <summary>
/// Parses Nutanix "Hardware Only" quote PDFs — multi-quote files from which only the reseller-facing
/// "Quote D" section is extracted (anchored on the Quote D banner). Handles wrapped product-code
/// continuation rows and multi-page page-footer noise. Foreign-currency → "Foreign Uplift" output.
/// See docs/nutanix_hardware_only_pdf.md.
/// </summary>
public sealed partial class NutanixHardwareOnlyPdfParser : IParser
{
    ...
    /// <summary>
    /// Wrong-file-type hint (not routing): 0.9 when the reseller-facing "Quote D" banner is present —
    /// unique to the multi-quote Hardware Only file among the Nutanix PDF formats.
    /// </summary>
    ...
    public ParseResult Parse(string path)
    {
        ...
        var bid = BidMetadataCleaner.CleanRevisionless(ExtractParentQuoteBidNumber(words, bannerIndex));
        if (bid.BidNumber is null) bid = BidMetadataCleaner.FromFilename(path);
    ...
    private static LineItem BuildItem(CurrentItem item)
    {
        ...
        return new LineItem
        {
            Vpn = NutanixVpnNormalizer.Normalize(JoinProductCode(item.CodeParts)),
    ...
    /// <summary>
    /// This is a multi-quote document, so take the <em>last</em> "Quote: XQ-… - Parent Quote" header
    /// preceding the Quote D banner — the nearest one above the section actually being extracted.
    /// </summary>
    private static string ExtractParentQuoteBidNumber(IReadOnlyList<PdfWord> words, int bannerIndex)
    {
        var text = PdfTableHelpers.WordStreamText(words.Take(bannerIndex));
        var matches = ParentQuoteField().Matches(text);
        return matches.Count == 0 ? string.Empty : matches[^1].Groups[1].Value;
    }

    [GeneratedRegex(@"Quote\s*:\s*(XQ-\d+)\s*-\s*Parent\s+Quote", RegexOptions.IgnoreCase)]
    private static partial Regex ParentQuoteField();
