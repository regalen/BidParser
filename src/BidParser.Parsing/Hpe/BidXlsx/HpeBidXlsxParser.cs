...
/// <summary>
/// Parses HPE "Bid" quote spreadsheets (XLSX). Anchor-based on the one-word "LineType" header (distinct
/// from HP Bid's "Line Type"); handles Part / Bundle / Bundle-Detail line types with sub-sequencing,
/// like HP Bid. Emits ANZ-GENERIC output. HPE is its own vendor with a single XLSX format, so it needs
/// no Detect override. See docs/hpe_bid_xlsx.md.
/// </summary>
public sealed class HpeBidXlsxParser : IParser
{
    ...
    public ParseResult Parse(string path)
    {
        ...
        // Locate DealNumber / DealVersion from the metadata block above the header (key in this
        // column, value in the next).
        var dealNumber = WorkbookReader.ValueRightOf(sheet, "DealNumber");
        var dealVersion = WorkbookReader.ValueRightOf(sheet, "DealVersion");
