...
/// <summary>
/// Parses HP "OneConfig" quote spreadsheets (XLSX). Anchor-based on the "Config ID" header; reads the
/// config totals and line items. Emits the "% Off RRP with Uplift" output (requires IM% at parse time).
/// See docs/hp_oneconfig_xlsx.md.
/// </summary>
public sealed class HpOneConfigXlsxParser : IParser
{
    ...
    /// <summary>
    /// Wrong-file-type hint (not routing): 0.9 when the "Config ID" header — unique to the HP
    /// OneConfig export — is present.
    /// </summary>
    ...
    public ParseResult Parse(string path)
    {
        ...
        var bid = BidMetadataCleaner.CleanRevisionless(configId);
