...
/// files (Lenovo LBP-E ISG and Cisco CCW) continue to pass the OLE check unchanged.
/// </summary>
public sealed class ZebraPriceConcessionXlsParser : IParser
{
    public string Slug => ParserSlugs.ZebraPcrXls;
    public string DisplayName => "PCR (XLS)";
    ...
    /// <summary>
    /// Recognition signature for the Zebra Price Concession XLS export (HTML under a .xls name):
    /// ≥2 HTML tables plus an items header row carrying "Part No." and "Unit Special Price".
    /// Returns 0.0 for real OLE .xls workbooks (Lenovo/Cisco) — they contain no parseable HTML table.
    /// </summary>
    public double Detect(string path)
    {
        try
        {
            ...
            if (tables is null || tables.Count < 2) return 0.0;

            var hasItemsHeader = tables
                .SelectMany(t => t.SelectNodes(".//tr") ?? Enumerable.Empty<HtmlNode>())
                .Select(CellTexts)
                .Any(cells =>
                    cells.Any(c => c.Contains("Part No.", StringComparison.OrdinalIgnoreCase))
                    && cells.Any(c => c.Contains("Unit Special Price", StringComparison.OrdinalIgnoreCase)));

            if (!hasItemsHeader) return 0.0;

            var hasConcessionAnchor = doc.DocumentNode.InnerText
                .Contains("Price Concession", StringComparison.OrdinalIgnoreCase);

            return hasConcessionAnchor ? 0.85 : 0.75;
        }
        catch
        {
            return 0.0;
