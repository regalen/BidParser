...
This repo is at the planning stage. No source code has been scaffolded yet. The committed artefacts are:
...
- `docs/software_only_pdf.md` — human-written extraction spec for the "Software Only (PDF)" format (Nutanix subscription quotes).
- `docs/software_only_xlsx.md` — human-written extraction spec for the "Software Only (XLSX)" format (same data as Software Only PDF, delivered as a workbook).
- `docs/renewal_pdf.md` — human-written extraction spec for the "Renewal (PDF)" format (Nutanix subscription renewals with serial/license numbers).
- `docs/hardware_only_pdf.md` — human-written extraction spec for the "Hardware Only (PDF)" format (Nutanix multi-quote PDF; we only parse Quote D, the reseller-facing breakdown).
- `docs/hardware_only_xlsx.md` — human-written extraction spec for the "Hardware Only (XLSX)" format (Nutanix multi-quote workbook; we only parse Quote D, the reseller-facing breakdown — same sub-quote as the PDF version).
...
- `XQ-4108785.pdf` and `XQ-4108785.xlsx` are both multi-quote files (sections Quote A, B, C, D). Both parsers extract **Quote D only** (`hardware_only_pdf.md` and `hardware_only_xlsx.md`) — the reseller-facing breakdown. Quote C inside these files is a separate budgetary breakdown of pure components and is **not** parsed by either format. Quotes A and B are noise.

## Commands

No build/test commands yet — backend, frontend, and Docker config have not been scaffolded. When they are, they will live at:

- `backend/` — FastAPI app, tested with `pytest -q` (run from inside `backend/` or `docker compose exec backend pytest -q`).
- `frontend/` — Vite + React + TypeScript, dev with `npm run dev`, build with `npm run build`.
- Root `docker-compose.yml` — `docker compose up --build` serves the frontend at `http://localhost:8080` proxying `/api` to the backend.
