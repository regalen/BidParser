import type {
  DellApiSettings,
  DellApiSettingsUpdate,
  HistoryResponse,
  MetricsSummaryResponse,
  MonitoringRunsResponse,
  ParserInfo,
  ParseUiConfig,
  RuntimeConfigurationAdmin,
...
export interface ParseResponse {
  ...
  /** Workbook count from a Solution-ID ZIP response; null for a normal XLSX response. */
  ...
  /** Whether the source quote contains Dell parent/base SKUs ineligible for rebate/MDF. */
...
export const api = {
  ...
  parseUiConfig(): Promise<ParseUiConfig> {
    return request<ParseUiConfig>('/parse-ui-config');
  },

  runtimeConfiguration(): Promise<RuntimeConfigurationAdmin> {
    return request<RuntimeConfigurationAdmin>('/admin/runtime-config');
  },

  updateGuidanceMessages(json: string): Promise<RuntimeConfigDocument> {
    return request<RuntimeConfigDocument>('/admin/runtime-config/guidance-messages', {
      method: 'PUT', body: JSON.stringify({ json }),
    });
  },

  updateVendorDefaults(json: string): Promise<RuntimeConfigDocument> {
    return request<RuntimeConfigDocument>('/admin/runtime-config/vendor-defaults', {
      method: 'PUT', body: JSON.stringify({ json }),
    });
  },
  ...
  dellSettings(): Promise<DellApiSettings> {
    return request<DellApiSettings>('/admin/dell-settings');
  },

  updateDellSettings(payload: DellApiSettingsUpdate): Promise<DellApiSettings> {
    return request<DellApiSettings>('/admin/dell-settings', {
      method: 'PUT',
  ...
  async testDellSettings(): Promise<void> {
    await request('/admin/dell-settings/test', { method: 'POST' });
  },
  ...
  updateUser(id: number, payload: { username?: string; name?: string; role?: 'admin' | 'user'; resetPassword?: boolean }): Promise<UserWithTempPassword> {
  ...
  async parse(formData: FormData): Promise<ParseResponse> {
    ...
    const splitCountRaw = response.headers.get('X-Split-Count');
    const parsedSplitCount = splitCountRaw === null ? Number.NaN : Number.parseInt(splitCountRaw, 10);
    ...
    return {
      ...
      parserSlug: response.headers.get('X-Parser-Slug'),
      outputWorkbookCount: Number.isFinite(parsedSplitCount) ? parsedSplitCount : null,
      cancelledLines,
      hasRebateIneligibleItems: response.headers.get('X-Rebate-Ineligible') === 'true',
    };
  },
};
...
function handleAuthSideEffect(path: string, status: number, detail: unknown) {
  ...
  if (status === 403 && detail === 'passwordChangeRequired') {
