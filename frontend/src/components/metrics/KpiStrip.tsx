...
export function KpiStrip({ data }: { data: MetricsSummaryResponse | null }) {
  ...
  const { totalParses, activeUsers, activeVendors, mismatchRate } = data.kpis;
  const mismatchPct = (parseFloat(mismatchRate) * 100).toFixed(1);

  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
      <div className="card flex flex-col p-4">
        <span className="label text-slate-500">Total Parses</span>
        <span className="mt-2 text-2xl font-semibold text-slate-900">{totalParses}</span>
      </div>
      <div className="card flex flex-col p-4">
        <span className="label text-slate-500">Active Users</span>
        <span className="mt-2 text-2xl font-semibold text-slate-900">{activeUsers}</span>
      </div>
      <div className="card flex flex-col p-4">
        <span className="label text-slate-500">Active Vendors</span>
        <span className="mt-2 text-2xl font-semibold text-slate-900">{activeVendors}</span>
      </div>
      <div className="card flex flex-col p-4">
