...
type PresetKey = 'last_7' | 'last_30' | 'this_month' | 'last_month' | 'month' | 'custom' | 'all';
...
function detectPreset(range: string | null, from: string | null, to: string | null): PresetKey {
  if (range === 'all') return 'all';
  if (!from || !to) return 'last_30';
  for (const candidate of ['last_7', 'last_30', 'this_month', 'last_month'] as const) {
    const candidateRange = presetRange(candidate, null);
    if (candidateRange && candidateRange.from === from && candidateRange.to === to) return candidate;
  }
...
export function DateRangeControl() {
  ...
  const selected = useMemo(() => detectPreset(rangeParam, from, to), [rangeParam, from, to]);
  const monthValue = selected === 'month' && from ? from.slice(0, 7) : '';

  function applyRange(mode: 'all' | 'bounded' | null, range: { from: string; to: string } | null) {
    const next = new URLSearchParams(searchParams);
    next.delete('page'); // Always reset page on date change
    if (mode === 'all') {
      next.set('range', 'all');
      next.delete('from');
      next.delete('to');
  ...
  function handlePresetChange(preset: PresetKey) {
    ...
    if (preset === 'custom') {
      ...
      applyRange('bounded', { from: toIso(def), to: toIso(today) });
      return;
    }
    if (preset === 'month') {
      const today = new Date();
      ...
      if (range) applyRange('bounded', range);
      return;
    }
