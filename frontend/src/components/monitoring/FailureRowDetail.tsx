  ...
  return (
    <div className="space-y-4 bg-slate-50 px-6 py-4">
      <dl className="grid grid-cols-1 gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
        ...
        {failure.stage && <Field label="Stage">{failure.stage}</Field>}
        {failure.hint && <Field label="Hint">{failure.hint}</Field>}
        {failure.message && <Field label="Message">{failure.message}</Field>}
      </dl>

      <div className="rounded-md border border-slate-200 bg-white">
        <div className="flex items-center justify-between border-b border-slate-200 px-3 py-2">
          <span className="text-xs font-semibold uppercase tracking-wide text-slate-500">Stack trace</span>
          <button
            type="button"
            onClick={handleCopy}
