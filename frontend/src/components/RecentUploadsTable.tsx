...
export function RecentUploadsTable({
  ...
  return (
    <section ref={tableRef} className="card mt-5 flex min-h-[360px] flex-1 flex-col overflow-hidden">
      ...
      <div className="hidden grid-cols-[0.8fr_minmax(180px,1.4fr)_1.2fr_1.1fr_0.7fr_84px] gap-3 border-b border-slate-200 bg-slate-50 px-4 py-2 md:grid">
        <span className="label-faint">Vendor</span>
        <span className="label-faint">Bid number</span>
...
function displayBidNumber(row: HistoryRow) {
  if (!row.bidNumber || !row.bidRevision) return '—';
  return `${row.bidNumber} v${row.bidRevision}`;
}
...
function UploadRow({ row, last }: { row: HistoryRow; last: boolean }) {
  return (
    <div
      className={
        'grid gap-3 px-4 py-3 text-[13px] md:grid-cols-[0.8fr_minmax(180px,1.4fr)_1.2fr_1.1fr_0.7fr_84px] md:items-center ' +
        (last ? '' : 'border-b border-slate-100')
      }
    >
      <span className="font-semibold text-slate-700">{row.vendor}</span>
      <span className="min-w-0 truncate font-semibold text-slate-900" title={displayBidNumber(row)}>{displayBidNumber(row)}</span>
      <span className="text-slate-600">{row.fileTypeDisplay}</span>
      <span className="truncate text-slate-600" title={row.crmTemplate}>{row.crmTemplate}</span>
