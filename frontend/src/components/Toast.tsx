...
const toneBorder: Record<ToastMessage['tone'], string> = {
  success: 'border-l-emerald-500',
  warning: 'border-l-amber-500',
  error: 'border-l-red-500',
};

const iconColor: Record<ToastMessage['tone'], string> = {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 flex w-[min(420px,calc(100vw-32px))] flex-col gap-2">
      {toasts.map((toast) => (
        ...
        >
          {toast.tone === 'success' ? (
            <CheckCircle2 className={'mt-0.5 h-5 w-5 shrink-0 ' + iconColor[toast.tone]} />
          ) : (
            <AlertTriangle className={'mt-0.5 h-5 w-5 shrink-0 ' + iconColor[toast.tone]} />
          )}
