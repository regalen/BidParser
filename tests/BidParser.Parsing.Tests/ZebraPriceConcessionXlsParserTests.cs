...
public sealed class ZebraPriceConcessionXlsParserTests
{
    ...
    {
        var parser = new ParserRegistry().Parsers.Single(p => p.Slug == ParserSlugs.ZebraPcrXls);
        var result = parser.Parse(Sample("Zebra_PC_97000001.xls"));
        ...
        result.Metadata.ParserSlug.Should().Be(ParserSlugs.ZebraPcrXls);
    }
    ...
    // ── PC# 97000002 — 5 items (media/wristband consumables) ─────────────────

    [Fact]
    public void File2_ItemCount_IsFive()
    {
        var parser = new ParserRegistry().Parsers.Single(p => p.Slug == ParserSlugs.ZebraPcrXls);
        var result = parser.Parse(Sample("Zebra_PC_97000002.xls"));
    ...
    {
        var parser = new ParserRegistry().Parsers.Single(p => p.Slug == ParserSlugs.ZebraPcrXls);
        var result = parser.Parse(Sample("Zebra_PC_97000003.xls"));
