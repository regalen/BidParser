...
/// <summary>
/// Characterisation tests for <c>ZebraPriceConcessionXlsParser</c>.
/// The .xls files are HTML tables (Zebra's portal exports HTML under a .xls extension).
/// Values are derived from the three sample XLS exports (matching the PDF samples).
/// </summary>
public sealed class ZebraPriceConcessionXlsParserTests
{
    ...
    {
        var parser = new ParserRegistry().Parsers.Single(p => p.Slug == ParserSlugs.ZebraPriceConcessionXls);
        var result = parser.Parse(Sample("Zebra_PC_81391641.xls"));
        ...
        result.Metadata.ParserSlug.Should().Be(ParserSlugs.ZebraPriceConcessionXls);
    }
    ...
    [Fact]
    public void File1_FirstItem_FieldsMatchPdf()
    {
        // XLS and PDF parsing should produce identical field values for the same file.
        var parser = new ParserRegistry().Parsers.Single(p => p.Slug == ParserSlugs.ZebraPriceConcessionXls);
        var result = parser.Parse(Sample("Zebra_PC_81391641.xls"));
        ...
        item.Description.Should().Contain("DS8178-HC FIPS DESK/WALL CRADLE");
    }
    ...
    // ── PC# 81413855 — 5 items (media/wristband consumables) ─────────────────

    [Fact]
    public void File2_ItemCount_IsFive()
    {
        var parser = new ParserRegistry().Parsers.Single(p => p.Slug == ParserSlugs.ZebraPriceConcessionXls);
        var result = parser.Parse(Sample("Zebra_PC_81413855.xls"));

        result.LineItems.Should().HaveCount(5);
    }

    [Fact]
    public void File2_AllItems_AreActive()
    {
        var parser = new ParserRegistry().Parsers.Single(p => p.Slug == ParserSlugs.ZebraPriceConcessionXls);
        var result = parser.Parse(Sample("Zebra_PC_81413855.xls"));

        result.LineItems.Should().AllSatisfy(i => i.IsCancelled.Should().BeFalse());
    }
    ...
    {
        var parser = new ParserRegistry().Parsers.Single(p => p.Slug == ParserSlugs.ZebraPriceConcessionXls);
        var result = parser.Parse(Sample("Zebra_PC_81422095.xls"));

        result.LineItems.Should().HaveCount(8);
    }

    [Fact]
    public void File3_PageBreakItem_MatchesPdfExtraction()
    {
        // The XLS has no page breaks, so the description is unambiguously in one row.
        var parser = new ParserRegistry().Parsers.Single(p => p.Slug == ParserSlugs.ZebraPriceConcessionXls);
        var result = parser.Parse(Sample("Zebra_PC_81422095.xls"));
