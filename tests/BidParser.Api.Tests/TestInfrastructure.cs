...
internal sealed class CustomTestFixture : IDisposable
{
    ...
    public static async Task<CustomTestFixture> CreateAsync(
        ...
        IDellQuoteClient? dellQuoteClient = null)
    {
        ...
        var factory = new WebApplicationFactory<Program>().WithWebHostBuilder(builder =>
        {
            ...
            if (registry is not null || configureServices is not null || dellQuoteClient is not null)
            {
                builder.ConfigureServices(services =>
                {
                    ...
                    if (dellQuoteClient is not null)
                    {
                        var d = services.SingleOrDefault(s => s.ServiceType == typeof(IDellQuoteClient));
                        if (d is not null) services.Remove(d);
                        services.AddSingleton(dellQuoteClient);
                    }
