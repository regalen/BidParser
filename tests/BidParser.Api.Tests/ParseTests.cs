...
public sealed class ParseTests
{
    ...
    public async Task ParseRoundtripDownloadsFile()
    {
        ...
        using var response = await PostParseAsync(client, bytes, "XQ-9100002.pdf", "application/pdf",
            "Nutanix", "nutanix_software_only_pdf", "0.7400", "7.50");

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        response.Content.Headers.ContentDisposition.Should().NotBeNull();
        response.Content.Headers.ContentDisposition!.DispositionType.Should().Be("attachment");
        response.Content.Headers.ContentDisposition.FileName!.Trim('"').Should().Be("XQ-9100002_1_ForeignUplift.xlsx");
    ...
    public async Task ParseErrors_UnsupportedExtension()
    {
        ...
        (await ApiTestFixture.DetailAsync(response)).Should().Be("Only PDF, XLS, XLSX, XLSM, and JSON files are supported.");
    }

    [Fact]
    public async Task ParseRoundtrip_ManualXlsx_Lenovo_DownloadsFile_And_PersistsManualJob()
    {
        using var fixture = await ApiTestFixture.CreateAsync();
        using var client = fixture.Factory.CreateClient();
        await ApiTestFixture.UnlockAdminAsync(client);
        ...
        var root = FindRepoRoot();
        const string filename = "Bid_Platform_Bid_Request_Sample_04.xlsx";
        var bytes = File.ReadAllBytes(Path.Combine(root, "samples", "inputs", filename));

        using var response = await PostParseAsync(
            client, bytes, filename, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            Vendors.LenovoIsg, ParserSlugs.LenovoLbpeIsgXls, "1.0000", "0.00");

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        response.Content.Headers.ContentDisposition.Should().NotBeNull();
        response.Content.Headers.ContentDisposition!.FileName!.Trim('"')
            .Should().Be("BRDADTEST0001_1_NoCalculation.xlsx");
        response.Headers.GetValues("X-Validation").Should().ContainSingle().Which.Should().Be("match");
        response.Headers.GetValues("X-Computed-Total").Should().ContainSingle().Which.Should().Be("71380.91");
        response.Headers.GetValues("X-Quoted-Total").Should().ContainSingle().Which.Should().Be("71380.91");
        ...
        job.SourceFilename.Should().Be(filename);
    }
    ...
    [Fact]
    public async Task ParseErrors_MagicBytesMismatch_ZipNamedXls()
    {
        using var fixture = await ApiTestFixture.CreateAsync();
        using var client = fixture.Factory.CreateClient();
        await ApiTestFixture.UnlockAdminAsync(client);

        var zipRenamedToXls = new byte[] { 0x50, 0x4B, 0x03, 0x04, 0x14, 0x00 };
        using var response = await PostParseAsync(client, zipRenamedToXls, "corrupt.xls", "application/vnd.ms-excel",
            Vendors.LenovoIsg, ParserSlugs.LenovoLbpeIsgXls, "1.0", "5.0");
    ...
    [Fact]
    public async Task ParseErrors_MagicBytesMismatch_OleNamedXlsx()
    {
        using var fixture = await ApiTestFixture.CreateAsync();
        using var client = fixture.Factory.CreateClient();
        await ApiTestFixture.UnlockAdminAsync(client);

        var oleRenamedToXlsx = new byte[] { 0xD0, 0xCF, 0x11, 0xE0, 0xA1, 0xB1, 0x1A, 0xE1 };
        using var response = await PostParseAsync(client, oleRenamedToXlsx, "corrupt.xlsx",
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            Vendors.LenovoIsg, ParserSlugs.LenovoLbpeIsgXls, "1.0", "5.0");
    ...
    [Fact]
    public async Task ParseRebateIneligibleItems_SetsWarningHeader()
    {
        var warningParser = new TestParser(
            "test-rebate-warning", "TestVendor", "application/pdf",
            _ => new ParseResult
            {
                Metadata = new QuoteMetadata
                {
                    ...
                    ParserSlug = "test-rebate-warning"
                },
                ...
                HasRebateIneligibleItems = true
            });

        using var fixture = await CustomTestFixture.CreateAsync(new TestRegistry(warningParser));
        using var client = fixture.Factory.CreateClient();
        await ApiTestFixture.UnlockAdminAsync(client);

        using var response = await PostParseAsync(client, MinimalPdfBytes(), "test.pdf", "application/pdf",
            "TestVendor", "test-rebate-warning", "1.0", "5.0");

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        response.Headers.GetValues("X-Rebate-Ineligible").Should().ContainSingle().Which.Should().Be("true");
    }
    ...
    public async Task ForwardedHeadersIgnoreUntrustedSource()
    {
        ...
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var json = await response.Content.ReadFromJsonAsync<JsonElement>();
        json.GetProperty("remoteIp").GetString().Should().NotBe("1.2.3.4");
    ...
    public async Task ForwardedHeadersAllowConfiguredProxy()
    {
        ...
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var json = await response.Content.ReadFromJsonAsync<JsonElement>();
        json.GetProperty("remoteIp").GetString().Should().Be("1.2.3.4");
    ...
    {
        // Parse persists last-used vendor on the User row, but fxRate / margin /
        // imPercent are intentionally NOT persisted — the dashboard re-prompts
        ...
        using var parseResponse = await PostParseAsync(client, bytes, "XQ-9100002.pdf", "application/pdf",
            "Nutanix", "nutanix_software_only_pdf", "0.6543", "8.75");
        parseResponse.StatusCode.Should().Be(HttpStatusCode.OK);

        var me = await client.GetFromJsonAsync<JsonElement>("/api/me");
        me.GetProperty("defaultVendor").GetString().Should().Be("Nutanix");
        me.GetProperty("fxRate").ValueKind.Should().Be(JsonValueKind.Null);
    ...
    [Fact]
    public async Task ParseWithNonAsciiSourceFilenameUsesSafeFallbackOutputName()
    {
        // Regression (H1): a filename with a non-ASCII character (en-dash, common in
        // files saved from Outlook/Windows) must not blow up the response. When a parser cannot
        // recover bid metadata from the document, the fallback name must still be safe for a
        // response header.
        using var fixture = await ApiTestFixture.CreateAsync();
        using var client = fixture.Factory.CreateClient();
        await ApiTestFixture.UnlockAdminAsync(client);
        ...
        response.Content.Headers.ContentDisposition.Should().NotBeNull();
        response.Content.Headers.ContentDisposition!.FileName!.Trim('"')
            .Should().Be("Quote _ Test_ForeignUplift.xlsx");
    ...
    {
        // A double-quote in the filename must not corrupt the
        // Content-Disposition header. fileDownloadName escapes it.
        using var fixture = await ApiTestFixture.CreateAsync();
        using var client = fixture.Factory.CreateClient();
        await ApiTestFixture.UnlockAdminAsync(client);
    ...
    [Fact]
    public async Task HistoryDownloadsRetainNonAsciiSourceAndUseSafeFallbackOutput()
    {
        ...
        output.Content.Headers.ContentDisposition!.FileName!.Trim('"')
            .Should().Be("Quote _ Test_ForeignUplift.xlsx");
    }
    ...
    public async Task DefaultVendorValidationTest()
    {
        ...
        // A vendor string no parser is registered under is rejected. Deliberately not a real
        // vendor name — every name in Vendors.* is expected to be accepted here.
        var bad = await ApiTestFixture.PatchJsonWithCsrfAsync(client, "/api/me/settings",
            new { DefaultVendor = "Acme" });
        ...
        var good = await ApiTestFixture.PatchJsonWithCsrfAsync(client, "/api/me/settings",
            new { DefaultVendor = "Nutanix" });
        good.StatusCode.Should().Be(HttpStatusCode.OK);
        var json = await good.Content.ReadFromJsonAsync<JsonElement>();
        json.GetProperty("defaultVendor").GetString().Should().Be("Nutanix");

        // Every vendor a parser is registered under must be accepted (not 400) — ParseService
        // persists these on a successful parse, so the allowlist has to cover all of them.
                 ...
                 })
        {
            var res = await ApiTestFixture.PatchJsonWithCsrfAsync(client, "/api/me/settings",
                new { DefaultVendor = vendor });
            res.StatusCode.Should().Be(HttpStatusCode.OK);
            (await res.Content.ReadFromJsonAsync<JsonElement>()).GetProperty("defaultVendor").GetString().Should().Be(vendor);
        }
        ...
        (await fx.Content.ReadFromJsonAsync<JsonElement>()).GetProperty("fxRate").GetString().Should().Be("1.2345");
    }
    ...
    public async Task ParseRejectsCurrencyNotationFxRate()
    {
        ...
        response.StatusCode.Should().Be(HttpStatusCode.BadRequest);
        (await ApiTestFixture.DetailAsync(response)).Should().Be("Invalid fxRate.");
    }
