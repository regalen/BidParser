...
public sealed class AuthFlowTests
{
    ...
    public async Task BootstrapLoginAndForcedPasswordChangeGate()
    {
        ...
        loginJson.GetProperty("user").GetProperty("mustChangePassword").GetBoolean().Should().BeTrue();

        var blocked = await client.GetAsync("/api/parsers");
        blocked.StatusCode.Should().Be(HttpStatusCode.Forbidden);
        (await ApiTestFixture.DetailAsync(blocked)).Should().Be("passwordChangeRequired");

        var weak = await ApiTestFixture.PostJsonWithCsrfAsync(client, "/api/auth/change-password", new { OldPassword = "changeme", NewPassword = "weak" });
        ...
        var changed = await ApiTestFixture.PostJsonWithCsrfAsync(client, "/api/auth/change-password", new { OldPassword = "changeme", NewPassword = "Admin123!" });
    ...
    public async Task PasswordChangeRevokesOtherSessionsButKeepsCurrent()
    {
        ...
        var temp = (await create.Content.ReadFromJsonAsync<JsonElement>()).GetProperty("tempPassword").GetString()!;
        ...
        (await ApiTestFixture.PostJsonWithCsrfAsync(s1, "/api/auth/change-password",
            new { OldPassword = temp, NewPassword = "UserB123!" })).EnsureSuccessStatusCode();
        ...
        (await ApiTestFixture.PostJsonWithCsrfAsync(s1, "/api/auth/change-password",
            new { OldPassword = "UserB123!", NewPassword = "UserB456!" })).EnsureSuccessStatusCode();
        (await s1.GetAsync("/api/me")).StatusCode.Should().Be(HttpStatusCode.OK);
    ...
    public async Task AdminPasswordResetRevokesUserSession()
    {
        ...
        var reset = await ApiTestFixture.PatchJsonWithCsrfAsync(admin, $"/api/users/{userId}", new { ResetPassword = true });
    ...
    public async Task LoginIsCaseInsensitiveAndSettingsPreserveDecimalScale()
    {
        ...
        var settings = await ApiTestFixture.PatchJsonWithCsrfAsync(client, "/api/me/settings", new { DefaultVendor = "Nutanix", FxRate = "0.74", margin = "7.5" });
        ...
        json.GetProperty("fxRate").GetString().Should().Be("0.7400");
        ...
        meJson.GetProperty("fxRate").GetString().Should().Be("0.7400");
    ...
    public async Task LockedUserPolicyAllowsOnlyLoggedInEndpoints()
    {
        ...
        var weakChange = await ApiTestFixture.PostJsonWithCsrfAsync(client, "/api/auth/change-password", new { OldPassword = "changeme", NewPassword = "weak" });
        weakChange.StatusCode.Should().Be(HttpStatusCode.BadRequest);

        var lockedEndpoints = new List<Task<HttpResponseMessage>>
        {
            ApiTestFixture.PatchJsonWithCsrfAsync(client, "/api/me/settings", new { DefaultVendor = "Nutanix" }),
        ...
        foreach (var response in await Task.WhenAll(lockedEndpoints))
        {
            ...
            (await ApiTestFixture.DetailAsync(response)).Should().Be("passwordChangeRequired");
        }
    ...
    public async Task FrameworkAndCsrfErrorsUseDetailShape()
    {
        ...
        (await ApiTestFixture.DetailAsync(missingCsrf)).Should().Be("csrfRequired");
...
internal sealed class ApiTestFixture : IDisposable
{
    ...
    public static async Task UnlockAdminAsync(HttpClient client)
    {
        ...
        var changed = await PostJsonWithCsrfAsync(client, "/api/auth/change-password", new { OldPassword = "changeme", NewPassword = "Admin123!" });
    ...
    public static async Task UnlockUserAsync(HttpClient client)
    {
        ...
        var tempPassword = createdJson.GetProperty("tempPassword").GetString()!;

        var login = await PostJsonWithCsrfAsync(client, "/api/auth/login", new { username = "user1", password = tempPassword });
        login.EnsureSuccessStatusCode();

        var changed = await PostJsonWithCsrfAsync(client, "/api/auth/change-password", new { OldPassword = tempPassword, NewPassword = "User123!" });
        changed.EnsureSuccessStatusCode();
    }
