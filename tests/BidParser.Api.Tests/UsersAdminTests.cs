...
public sealed class UsersAdminTests
{
    ...
    public async Task AdminUserCrudAndLastAdminGuards()
    {
        ...
        created.GetProperty("user").GetProperty("mustChangePassword").GetBoolean().Should().BeTrue();
        created.GetProperty("tempPassword").GetString().Should().HaveLength(10);
        ...
        updated.GetProperty("tempPassword").ValueKind.Should().Be(JsonValueKind.Null);

        var reset = await ApiTestFixture.PatchJsonWithCsrfAsync(client, $"/api/users/{userId}", new { ResetPassword = true });
        reset.StatusCode.Should().Be(HttpStatusCode.OK);
        var resetJson = await reset.Content.ReadFromJsonAsync<JsonElement>();
        resetJson.GetProperty("user").GetProperty("mustChangePassword").GetBoolean().Should().BeTrue();
        resetJson.GetProperty("tempPassword").GetString().Should().HaveLength(10);
    ...
    public async Task NonAdminCannotUseUsersEndpoints()
    {
        ...
        var changed = await ApiTestFixture.PostJsonWithCsrfAsync(client, "/api/auth/change-password", new { OldPassword = tempPassword, NewPassword = "Sales123!" });
        ...
        (await ApiTestFixture.DetailAsync(users)).Should().Be("adminRequired");
    }
}
