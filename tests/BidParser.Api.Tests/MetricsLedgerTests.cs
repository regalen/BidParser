...
public sealed class MetricsLedgerTests
{
    private static Task<HttpResponseMessage> ParseAsync(HttpClient client, string sourcePdf, string parserSlug)
    {
        ...
        form.Add(new StringContent("1.52"), "fxRate");
    ...
    public async Task SuccessfulParseWritesParseMetricAndParseJob()
    {
        ...
        var sourcePdf = Path.Combine(root, "samples", "inputs", "XQ-9100002.pdf");
        ...
        metric.SourceFilename.Should().Be("XQ-9100002.pdf");
    ...
    public async Task DeleteUserLeavesMetricIntact()
    {
        ...
        var user2Temp = user2.GetProperty("tempPassword").GetString()!;
        ...
        var changeRes = await ApiTestFixture.PostJsonWithCsrfAsync(client, "/api/auth/change-password", new { OldPassword = user2Temp, NewPassword = "NewPassword1!" });
