...
- The vendor-specific settings block (Nutanix `EXCHANGE RATE` + `UPLIFT` inputs and the emerald `CRM IMPORT TEMPLATE` callout) renders in the dashboard only when **both** a vendor and a file type are selected. Until then, only the two cascading selects appear above the Upload & parse button.
- The `File type` dropdown (`FileTypeSelect.tsx`) renders a `Sample File` link beneath it once a parser is chosen, pointing at a static copy of a representative supplier quote under `/samples/<name>`. The mapping lives in the `SAMPLE_FILES` record at the top of `FileTypeSelect.tsx`; every parser slug should have an entry, and every entry must have a matching file committed under `frontend/public/samples/` (Vite serves it in dev; the ASP.NET app serves it from `wwwroot/` in prod). When adding a new parser, add both the `SAMPLE_FILES` entry and the file.
- UI label vocabulary differs from the underlying field names: the `margin` input is rendered as **`Uplift`** in both `NutanixSettingsBlock` and `HpSettingsBlock` (the percentage is added on top of cost, framed as an uplift to the seller); the HP OneConfig `im_percent` input is rendered as **`Discount Off MSRP`** (it represents the customer-facing discount, not an internal margin). The API request fields (`margin`, `im_percent`) and the database columns (`parse_jobs.margin`, `users.im`, `parse_metrics.margin`) are unchanged — the rename is presentation-only.
...
- All 176 tests pass (`dotnet test BidParser.sln`): 104 parsing + 72 API integration.
...
- **Per-user remembered vendor** — only the last-used vendor is persisted on each User row (via `ParseService` setting `user.DefaultVendor = vendor` on successful parse). The numeric inputs (FX rate / margin / IM%) are **never** persisted back to the User row from the parse flow — the dashboard re-prompts every parse so a stale numeric never gets silently re-applied. The User table still has nullable `fx_rate` / `margin` / `im` columns and the `/me/settings` PATCH endpoint still accepts them (used by tests / future tooling), but the SPA neither reads them on load nor writes them via that endpoint.
...
| `PATCH` | `/me/settings` | user | Body `{default_vendor?, fx_rate?, margin?, im_percent?}`. Endpoint still accepts the numerics so tests / future tooling can write them, but the SPA does not call it with numerics — only `default_vendor` is currently mutated, and that mutation happens implicitly inside `ParseService` on each successful parse. |
...
| `GET` | `/history` | user | `?limit=&offset=&q=` — user-scoped. `q` is a case-insensitive substring filter on `source_filename`. Each row carries `crm_template` (e.g. `"Foreign Uplift"`, `"No Calculation"`, `"Uplift"`, `"% Off RRP with Uplift"`) — the template that was actually written at parse time, persisted on `ParseJob.CrmTemplate`. `when` is a server-computed relative time string ("just now", "5m ago", "Yesterday", "3 days ago", then absolute date). |
...
- GitHub Actions CI/CD (`.github/workflows/build.yml`) — every push and PR runs the `test` job (`dotnet test BidParser.sln`). The `build-and-push` job that produces the `linux/amd64` Docker image only runs for `v*` tag pushes, so pushing main + tagging a release no longer double-builds the same SHA. Each tagged release publishes three image tags to `ghcr.io`: `<semver>` (e.g. `0.4.0`), `sha-<short-sha>`, and `latest`. The `build-and-push` job sets `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24: 'true'` because the four pinned `docker/*` actions (`build-push-action@v6`, `login-action@v3`, `metadata-action@v5`, `setup-buildx-action@v3`) still ship with the Node 20 runtime — Node 20 is being deprecated on GitHub-hosted runners (forced to Node 24 on 2026-06-02, removed on 2026-09-16). Remove the env override once those actions ship majors that run natively on Node 24.
...
- `dotnet test BidParser.sln` — full test suite (176 tests: 104 parsing + 72 API integration).
