...
- `src/BidParser.Domain/` — `LineItem`, `QuoteMetadata`, `ValidationResult`, `ParseResult`, `ParseError`, `IParser`, `IParserRegistry`. `Constants/` centralises vendor (`Vendors.Nutanix`, `Vendors.Hp`, `Vendors.Lenovo`), CRM template (`CrmTemplates.ForeignUplift`, `CrmTemplates.NoCalculation`, `CrmTemplates.Uplift`), and parser slug (`ParserSlugs.NutanixSoftwareOnlyPdf`, …, `ParserSlugs.HpBidXlsx`, `ParserSlugs.LenovoBrdaDcgPdf`) string literals — never hand-roll these strings in new code. `IParser` exposes `AvailableTemplates` (default `[CrmTemplate]`) so parsers that support multiple output templates override it.
...
- `src/BidParser.Parsing/` — Cleaning helpers, PDF word collection via PdfPig (`PdfWordCollector`, `PdfPigWordSplitter`, `PdfTableHelpers`), XLSX helpers via ClosedXML (`WorkbookReader`, `HeaderMap`), five Nutanix parsers, one HP parser, one Lenovo parser (`LenovoBrdaDcgPdfParser`), explicit `ParserRegistry`.
- `src/BidParser.Output/` — `ForeignUpliftWriter` (Nutanix, ClosedXML), `AnzGenericWriter` (HP No Calculation / Uplift, ClosedXML), shared `TemplateLayout` (internal, 27-column header array used by both writers), `OutputNaming`. Both writers emit the `0.000001` zero-cost sentinel (the downstream import rejects a literal `0`): `ForeignUpliftWriter` on the Foreign Cost/MSRP columns (T/U), `AnzGenericWriter` on the local Cost column (I) — which is where every HP `Bundle Detail` lands, since its component price is dropped onto the `Bundle` parent.
...
- `tests/BidParser.Parsing.Tests/` — xUnit tests: cleaning helpers, all five Nutanix parsers + HP parser + Lenovo BRDA DCG parser against golden inputs, template writer cell-by-cell equivalence against `samples/outputs/`.
...
- `samples/inputs/` — real supplier quote files. Seven PDFs (`XQ-4076249.pdf`, `XQ-4108785.pdf`, `XQ-4128926.pdf`, `XQ-4157308.pdf`, `XQ-4165884.pdf`, `XQ-4166696.pdf`, `BRDAS010260417V1.pdf`), two Nutanix XLSXs (`XQ-4076249.xlsx`, `XQ-4108785.xlsx`), and two HP XLSXs (`Deals20260518T034809_HPI.xlsx`, `Deals20260518T043243_HPI.xlsx`). `XQ-4108785.pdf` and `XQ-4108785.xlsx` are the *same engagement* delivered in two envelopes; both parsers extract from **Quote D** (reseller-facing breakdown) within those files and produce matching totals (`USD 22,491.87`). `XQ-4157308.pdf` and `XQ-4165884.pdf` are both Software Only (PDF) samples using the **extended 9-column layout** (adds `Selected Start Date`, `Total Discount`, `Total Net Price`); `XQ-4157308.pdf` is the minimal case (1 line item, side-by-side `Product Code` header) and `XQ-4165884.pdf` is the heavy case (11 line items, stacked `Product`/`Code` header, wrapped multi-line SKUs). `BRDAS010260417V1.pdf` is the Lenovo BRDA DCG sample (2 configs, 165 items, AUD 393,231.78).
...
- All 129 tests pass as of the last stable checkpoint; new Lenovo parser adds ~15 more (`dotnet test BidParser.sln`): parsing tests + 67 API integration tests.
...
| `BRDAS010260417V1.pdf` | Lenovo BRDA DCG (PDF) | 165 items: 2 configs, 13 parents, 150 children; AUD 393,231.78 |
...
The HP Bid (XLSX) parser declares `CrmTemplate = "No Calculation"` and `AvailableTemplates = ["No Calculation", "Uplift"]`. The user selects the template from a dropdown in the UI at parse time. `AnzGenericWriter.Write(items, path, sheetName, includeMargin, margin, vendorName)` produces HP output for both templates; the only difference is that `Uplift` populates the Margin column (K) while `No Calculation` leaves it blank. `Bundle Detail` rows carry `cost = 0` (the component price is dropped because the `Bundle` parent holds the deal total); `AnzGenericWriter` writes the `0.000001` sentinel in the Cost column (I) for any zero cost, since the downstream import rejects a literal `0`. Nutanix's sentinel lives on the Foreign columns (T/U); HP's is on the local Cost column.
...
- `dotnet test BidParser.sln` — full test suite (129 tests: 62 parsing + 67 API integration).
