...
The HP Bid (XLSX) parser declares `CrmTemplate = "No Calculation"` and `AvailableTemplates = ["No Calculation", "Uplift"]`. The user selects the template from a dropdown in the UI at parse time. `AnzGenericWriter.Write(items, path, sheetName, includeMargin, margin, vendorName)` produces HP output for both templates; the only difference is that `Uplift` populates the Margin column (K) while `No Calculation` leaves it blank. `Bundle Detail` rows carry `cost = 0` (the component price is dropped because the `Bundle` parent holds the deal total); `AnzGenericWriter` writes the `0.000001` sentinel in the Cost column (I) for any zero cost, since the downstream import rejects a literal `0`. Nutanix's sentinel lives on the Foreign columns (T/U); HP Bid's is on the local Cost column.

The HP OneConfig (XLSX) parser declares `CrmTemplate = "% Off RRP with Uplift"` (single template — no dropdown). `PercentOffWithUpliftWriter.Write(items, path, margin, imPercent, vendorName)` produces the output. The parent Config row writes its real `Total Price` on column H (MSRP); all children write the `0.000001` sentinel on H. Column I (Cost) is intentionally blank. Column K (Margin) and column X (IM%) are written for every row and are required parse parameters. IM% is persisted as `User.ImPercent` alongside `Margin` and `FxRate`, serialised as `im_percent` in JSON responses.
