...
- `FailedParseJob` records post-save parse failures with the full exception (`ex.ToString()`) and retains the original source file for admin download. Lifecycle is tied to retention — both the row and its source file are purged together at `RETENTION_DAYS`. Written from `ParseService.ParseAsync`'s catch block via a dedicated `FailedParseJobRecorder` that uses a fresh `AppDbContext` scope (so the request-scoped context's pending `DefaultVendor`/`FxRate`/`Margin` updates from the success path are *not* persisted on failure). `SaveUploadAsync` runs **outside** the recorder's `try` so pre-save failures (413 oversize, IO error) produce no row. Powers `/admin/monitoring` (`/api/monitoring/failures`, `/api/monitoring/failures/{id}/source`).
...
- All 88 tests pass (`dotnet test BidParser.sln`): 30 parsing tests + 58 API integration tests.
...
| `GET` | `/monitoring/failures` | admin | `?limit=&offset=` (limit clamped 1–100, default 25). Newest-first. Items include `category` (snake_case: `magic_byte_mismatch` / `parser_error` / `unhandled_exception`), `parser_display_name` (looked up from `IParserRegistry`), inline `error_detail`, and `source_available` (live `File.Exists` check). |
