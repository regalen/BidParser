...
- `src/BidParser.Domain/` — `LineItem`, `QuoteMetadata`, `ValidationResult`, `ParseResult`, `ParseError`, `IParser`, `IParserRegistry`. `Constants/` centralises vendor (`Vendors.Nutanix`, `Vendors.Hp`, `Vendors.Lenovo`), CRM template (`CrmTemplates.ForeignUplift`, `CrmTemplates.NoCalculation`, `CrmTemplates.Uplift`), and parser slug (`ParserSlugs.NutanixSoftwareOnlyPdf`, …, `ParserSlugs.HpBidXlsx`, `ParserSlugs.LenovoBrdaDcgPdf`, `ParserSlugs.LenovoBrdaDcgXlsx`) string literals — never hand-roll these strings in new code. `IParser` exposes `AvailableTemplates` (default `[CrmTemplate]`) so parsers that support multiple output templates override it.
...
- `src/BidParser.Parsing/` — Cleaning helpers, PDF word collection via PdfPig (`PdfWordCollector`, `PdfPigWordSplitter`, `PdfTableHelpers`), XLSX helpers via ClosedXML (`WorkbookReader`, `HeaderMap`), legacy `.xls` reading via ExcelDataReader (Lenovo BRDA DCG XLSX only — registers `CodePagesEncodingProvider` for `.xls` string decoding), five Nutanix parsers, one HP parser, two Lenovo parsers (`LenovoBrdaDcgPdfParser`, `LenovoBrdaDcgXlsxParser`), explicit `ParserRegistry`.
...
- `tests/BidParser.Parsing.Tests/` — xUnit tests: cleaning helpers, all five Nutanix parsers + HP parser + both Lenovo BRDA DCG parsers (PDF and XLSX) against golden inputs, template writer cell-by-cell equivalence against `samples/outputs/`.
...
- All 157 tests pass (`dotnet test BidParser.sln`): 90 parsing + 67 API integration.
...
- `dotnet test BidParser.sln` — full test suite (157 tests: 90 parsing + 67 API integration).
