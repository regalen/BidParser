...
- `src/BidParser.Domain/` — `LineItem`, `QuoteMetadata`, `ValidationResult`, `ParseResult`, `ParseError`, `IParser`, `IParserRegistry`. `Constants/` centralises vendor (`Vendors.Nutanix`, `Vendors.Hp`), CRM template (`CrmTemplates.ForeignUplift`, `CrmTemplates.NoCalculation`, `CrmTemplates.Uplift`), and parser slug (`ParserSlugs.NutanixSoftwareOnlyPdf`, …, `ParserSlugs.HpBidXlsx`) string literals — never hand-roll these strings in new code. `IParser` exposes `AvailableTemplates` (default `[CrmTemplate]`) so parsers that support multiple output templates override it.
...
- `src/BidParser.Parsing/` — Cleaning helpers, PDF word collection via PdfPig (`PdfWordCollector`, `PdfPigWordSplitter`, `PdfTableHelpers`), XLSX helpers via ClosedXML (`WorkbookReader`, `HeaderMap`), five Nutanix parsers, one HP parser, explicit `ParserRegistry`.
...
- `tests/BidParser.Parsing.Tests/` — xUnit tests: cleaning helpers, all five Nutanix parsers + HP parser against golden inputs, template writer cell-by-cell equivalence against `samples/outputs/`.
...
- All 129 tests pass (`dotnet test BidParser.sln`): 62 parsing tests + 67 API integration tests.
