...
- Phase 8 complete: `NutanixRenewalPdfParser` ported, registered after the software parsers in the explicit registry, and line-item/serial/date/total equivalence test added for `samples/inputs/XQ-4128926.pdf`. Verification: `dotnet build BidParser.sln --configuration Release --no-restore`, focused renewal parser test, and full solution `dotnet test BidParser.sln --configuration Release --no-build --verbosity normal` passed. Local Docker build intentionally not run.
...
- Phase 13 complete: `RetentionService` (scoped, deletes expired ParseJob rows and their files), `RetentionBackgroundService` (sleep-first 24h cadence), `HistoryEndpoints` (`GET /api/history` with user scoping/q-filter/pagination, `GET /api/history/{id}/source|output` with cross-user 404), `RelativeWhen` verbatim port, and history/retention tests added. `JsonStringDecimalConverter` unsealed to allow `HistoryFxRateConverter`/`HistoryMarginConverter` subclasses for 4dp/2dp formatting on the non-nullable `HistoryRow` fields. Verification: `dotnet build BidParser.sln --configuration Release --no-restore` and `dotnet test BidParser.sln --configuration Release --no-build` passed (54 tests: 25 parsing + 29 API). Local Docker build intentionally not run.
- Phase 14 complete: `backend/` deleted, Python entries removed from `.gitignore`, Vite proxy target updated to `:5000`, `BASE_URL` removed from `docker-compose.yml`, `docs/DEPLOYMENT.md` rewritten for .NET (SESSION_SECRET semantics, `/data/dp-keys` persistence, MigratorHostedService, local dev commands), `AGENTS.md` updated (C# paths/tools throughout, parser extraction algorithm specs kept verbatim). Verification: `dotnet build BidParser.sln` and `dotnet test BidParser.sln` passed (54 tests). Local Docker build intentionally not run.
- Current checkpoint: re-platform complete. Working branch is `dotnet-refactor`. All 14 phases done.
...
| 13 | Complete | port/12b-history-retention | M | 6 | |
| 14 | Complete | port/13-cutover | S | docs + deletions | |
