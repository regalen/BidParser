...
|---|---|---|---|
| `Deals_Sample_01_HPI.xlsx` | 99010001 | 479 | Part Number, Bundle, Bundle Detail rows |
| `Deals_Sample_02_HPI.xlsx` | 99010002 | 5   | Part Number rows only |
...
| **description** | `Product Description`. For a `Bundle`, HP repeats the line's own `Product Number/ID` at the front of the description (`"55623728-HP EliteBook 8 G2a 14 …"`); that exact `"{Product Number/ID}-"` prefix is stripped (`DescriptionCleaner.StripLeadingCode`) since the VPN column already carries the id. `Part Number` is verbatim. | `Product Description` verbatim |
...
quantity) is not used as the line quantity; it is surfaced in `comments` as `"Max Qty: {n}"`.

**Bundle Detail pricing:** a Bundle Detail is a component breakdown of its Bundle. The Bundle line carries the deal price for the whole bundle, so each Bundle Detail's own `Price` is intentionally dropped (`cost = 0`) to avoid double-counting in the computed total. On export, `CrmWriter` writes the `0.0001` sentinel in the Cost column for these zero-cost lines because the downstream import rejects a literal `0` (and rounds the sentinel back to `0`).
...
### `Deals_Sample_02_HPI.xlsx` (Part Number only, 5 items)

| Seq | VPN | Description | Cost | Qty | MinQty | Comments |
|---|---|---|---|---|---|---|
| 1 | 5TW10AA | HP USB-C Dock G5 | 165.83 | 1 | 1 | Max Qty: 100 |
| 2 | 9D9S0UT | HP S5 Pro 527pq QHD MNTR | 336.70 | 1 | 1 | Max Qty: 100 |
| 3 | BV2Q6PT | HP Z2 TWR G1i U7265 32GB/1TB PC | 3393.88 | 1 | 1 | Max Qty: 100 |
| 4 | BQ4E3PT | HP EB8G1I14 U5 225U 14 16GB/512 PC | 2258.18 | 1 | 1 | Max Qty: 500 |
| 5 | BV8B6PT | HP ZB8G1i14 U5 225H 14 16GB/512 PC | 2160.00 | 1 | 1 | Max Qty: 250 |

Qty is `1` for every line (`Min Order Qty` is 0 on all rows → 1). Computed total: `8314.59`
(Σ cost × 1; no quoted total).

### `Deals_Sample_01_HPI.xlsx` (Part Number + Bundle + Bundle Detail, 479 items)
...
Item 4's source `Product Description` reads
`"55623728-HP EliteBook 8 G2a 14 (14" Touch, Ryzen 5, 16GB, 512GB)"`; the repeated id is stripped
and it exports as `HP EliteBook 8 G2a 14 (14" Touch, Ryzen 5, 16GB, 512GB)`. All 14 Bundle rows in
this file carry the prefix; none of the 3 Part Number or 462 Bundle Detail rows do.
...
See `docs/output_mapping.md` — HP section. Both templates use the ANZ-GENERIC 27-column layout (`CrmWriter`). The key difference between templates:
...
The values adjacent to `Deal Number` and `Deal Version` (matched case-insensitively) supply `BidNumber` and `BidRevision`. Best-effort: an absent `Deal Version` keeps the number with revision `1`; an absent `Deal Number` leaves both null. Neither fails the parse. `QuoteNumber` keeps its separate filename fallback.
