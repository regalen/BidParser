...
**Part Number**
From the `Product Code` column. Trim whitespace and convert the parsed value to uppercase so it matches the SAP SKU identifier. Plain source labels such as `Support-Term` and `Platform Integration` therefore emit as `SUPPORT-TERM` and `PLATFORM INTEGRATION`; `Raw` retains the source casing.
...
Compute the line total by multiplying `Sale Price` by `Quantity` for every kept row and summing. Bundled-component rows contribute `$0 × qty = $0`, the `Support-Term` row contributes `$0 × 60 = $0`, and the `Platform Integration` row contributes `$0 × 1 = $0`. The remaining priced rows must sum to the quoted total. For `XQ-9100003.xlsx` Quote D the sum is `22,491.87`, matching the quoted total exactly.

## Bid metadata

Within the Quote D section, the `Parent Quote Name` column supplies the first non-empty `BidNumber`; revision is `1`. It is the workbook's only statement of quote identity (the section banner carries no number), and it is read optionally — an absent column leaves the bid fields null rather than rejecting the file.
