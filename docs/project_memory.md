...
- In Docker, image defaults place `DATABASE_URL` and `UPLOAD_DIR` under `/data` (`/data/db.sqlite`, `/data/files`). Don't set these in `docker-compose.yml` unless changing internal layout. `docker-compose.yml` mounts `/data` from the default `bidparser-data` volume or a `DATA_DIR` bind mount.
- `RetentionBackgroundService` (sleep-first, 24h cadence) calls `RetentionService.CleanupOldParseJobsAsync` to delete expired `ParseJob` rows + source+output files, and expired `FailedParseJob` rows + source files, after `RETENTION_DAYS`. `ParseMetric` rows retained indefinitely — `parse_job_id` nulled by SQLite `ON DELETE SET NULL` when the parent `ParseJob` is purged.
...
| `GET` | `/parsers` | user | Returns the registry — each entry has `slug`, `display_name`, `vendor`, `accepted_mime`, `crm_template`, `available_templates`. Drives the vendor → file-type cascade. |
...
**`/parse` response headers** on success: `X-Validation: match | mismatch`, `X-Currency`, `X-Computed-Total`, `X-Quoted-Total`. `X-Currency` carries `QuoteMetadata.Currency` (`USD` Nutanix, `AUD` HP) — never hard-code `USD` in the UI. Match → green success toast + immediate download; mismatch → blocking amber `ValidationWarningModal` (auto-focused OK), download deferred until acknowledged. **`X-Quoted-Total` empty-header semantic:** when `QuotedTotal == null` the header is **present with an empty string value**, not omitted (the SPA distinguishes "header missing" from "header present, empty"). ASP.NET strips empty-string headers by default — preserve with `new StringValues(new string[] { "" })`.
...
- **Single vendor**: Nutanix only (registry-driven so Dell/Lenovo drop in later, but no other vendor ships in MVP).
- **Single-file upload** per parse. "DRAG MULTIPLE FILES TO BATCH PARSE" is a future-state affordance.
- **Auto-download flow**, no review screen. Upload & parse → progress panel → `_parsed.xlsx` auto-downloads. Validation server-side; match → green toast; mismatch → blocking `ValidationWarningModal` (OK) before download.
...
Out of scope: multi-file batch upload, CSV formats, vendors other than Nutanix, review/approve gate, multi-tenancy, email, SSO, audit log beyond ParseJob history.
...
- **Persistent state** lives under `/data` (`db.sqlite` + `dp-keys/` + `files/originals/` + `files/outputs/`). **`/data/dp-keys` must persist** — it holds the Data Protection keyring; losing it logs everyone out. `${DATA_DIR:-bidparser-data}:/data` defaults to a named volume; `DATA_DIR` swaps it for a bind mount.
