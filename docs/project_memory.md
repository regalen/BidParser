...
- `tests/BidParser.Desktop.Configuration.Tests/` — 118 cross-platform tests covering schema-v1 fallback/validation, safe guidance markup, bounded HTTP behavior, independent failures, numeric-field validation, and SemVer update decisions. Together the three projects make `dotnet test BidParser.sln` a 993-test suite.
...
the rest are nullable and populated per format:
  ...
  Trellix Quote (PDF):        Description, Msrp, SerialNumber, StartDate, EndDate, Comments, LineSequence
