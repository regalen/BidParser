...
- `src/BidParser.Parsing/` — Cleaning helpers, PDF word collection via PdfPig (`PdfWordCollector`, `PdfTableHelpers`), XLSX/XLSM helpers via ClosedXML (`WorkbookReader`, `HeaderMap`), ExcelDataReader for legacy OLE `.xls` (Lenovo LBP-E and Cisco CCW) and OpenXML `.xlsx` (Lenovo LBP-E), HTML-disguised `.xls` via HtmlAgilityPack (Zebra Price Concession XLS), JSON reader via `System.Text.Json` (Dell CTO & APOS Quote API responses), and 24 concrete parsers in the explicit `ParserRegistry`. Datalogic, Epson, and Strike use content-band PDF recovery; Trellix PDF resolves its dense table from per-page heading geometry, while Trellix XLSM reads an anchored workbook table.
...
| `Trellix_Quote_900001.pdf` | Trellix Quote (PDF) | Synthetic 9-item multi-page report; repeated headers, wrapped fields, Data Center layout; AUD 1,546.05 |
| `Trellix_Quote_900002.pdf` | Trellix Quote (PDF) | Synthetic 2-item alternate layout; blank printed line number, hardware quantity, populated serial; AUD 50.00 |
| `Trellix_Quote_900003.xlsm` | Trellix Quote (XLSM) | Synthetic 4-item workbook; displayed/underlying dates, non-unit selling term, hardware quantity, zero price; AUD 99.99 |
| `Trellix_Quote_900004.xlsm` | Trellix Quote (XLSM) | Synthetic 1-item shifted-column workbook with a hidden synthetic sheet; AUD 36.25 |
...
the rest are nullable and populated per format:
  ...
  HPE Bid (XLSX):              Description, Msrp, LineSequence
  ...
  Trellix Quote (PDF/XLSM):   Description, Msrp, SerialNumber, StartDate, EndDate, Comments, LineSequence
...
- **Candidate set / coverage.** Only vendors with ≥2 formats accepting the upload MIME can ever name a
  ...
  Hardware), **HP XLSX** (Bid/Global Bid/OneConfig). Lenovo, Zebra, and Trellix each have two formats
...
- **`Detect()` signatures** (each `try/catch → score`, `0.0` on failure):
  ...
  - *Trellix PDF:* Trellix identity plus `Distributor Quote Report` and the `Channel SKU` table header.
  - *Trellix XLSM:* visible Trellix quote sheet with anchored `Quote ID`, `Quote Currency`, and `Channel SKU` headers.
...
Supported vendors (Nutanix, Lenovo ISG, Zebra, Trellix, and Dell via `AutoDetectTypes`) expose an **"Auto (detect format)"** entry in the file-type dropdown, preselected by default when that vendor is selected. Lenovo IDG has no Auto entry — a single-format vendor, matching HP/HPE/Cisco precedent (see the vendor-split section below). For Nutanix, Lenovo, Zebra, and Trellix this is only a default — the user can still pick a specific format. **Dell is the exception: Auto is the only selectable entry.** `ParseSettingsCard` filters Dell's dropdown down to the Auto entry and disables it (greyed out, no manual override), and `ParseEndpoints` enforces the same rule server-side — any `/api/parse` POST with `vendor=Dell` and `parserSlug != dell_auto` is rejected with `400 "Dell only supports automatic file-type detection."`, regardless of whether it came through the SPA or a direct API call.

- **Sentinel slugs & resolution.** `ParserSlugs.NutanixAuto` (`nutanix_auto`), `ParserSlugs.LenovoAuto` (`lenovo_auto`), `ParserSlugs.ZebraAuto` (`zebra_auto`), `ParserSlugs.TrellixAuto` (`trellix_auto`), and `ParserSlugs.DellAuto` (`dell_auto`). `/api/parsers` synthesizes an entry with `displayName = "Auto (detect format)"` and compatible `availableTemplates` ahead of the vendor's first parser. Guidance is never keyed by a sentinel slug.
- **Backend flow (`ParseService`).** When `parserSlug` is an Auto slug (`nutanix_auto` / `lenovo_auto` / `zebra_auto` / `trellix_auto` / `dell_auto`):
  - Derives MIME from the source extension (`.pdf` → `application/pdf`, `.xlsx` → `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`, `.xlsm` → `application/vnd.ms-excel.sheet.macroEnabled.12`, `.xls` → `application/vnd.ms-excel`, `.json` → `application/json`). For Dell, `ParseEndpoints` first validates `quoteId`, fetches the raw JSON, and supplies it as `<quoteId>.json`; Dell API failures return 422 with stage `dellApi` before anything is persisted.
...
- **Frontend SPA.** `DashboardPage` preselects Auto for Nutanix, Lenovo, Zebra, Trellix, and Dell. Nutanix, Lenovo, Zebra, and Trellix supply an uploaded file; Dell supplies a quote ID whose fetched JSON response is detected as CTO or APOS. For Dell, `ParseSettingsCard` additionally disables the file-type dropdown and filters it to only the Auto entry — the user has no way to force `dell_cto_json`/`dell_apos_json` manually. On parse success, the SPA reads `X-Parser-Slug` from `api.parse()`, looks up the resolved parser for report-type guidance, and displays `Detected file type: <DisplayName>` in `ParseResultModal`.
...
Trellix Quote (PDF): the same two local-currency templates, but no On Cost capability or seeded guidance. The parser reads `Cost Per Unit` directly, resolves one positive quantity from hardware or software/support, derives unit MSRP from `Total MSRP / Qty`, and validates against `Total Distribution Cost`. See `docs/trellix_quote_pdf.md` for the two table layouts and wrapped values.

Trellix Quote (XLSM): the same templates, fields, and capabilities. The parser anchors the visible workbook table and reads displayed Start/End dates when the export also contains underlying date columns. It validates summed `Final Disti Cost` line totals against `Σ(Cost Per Unit × Qty)` because the workbook has no quote-level distribution total. See `docs/trellix_quote_xlsm.md`.
...
- **Vendors ship one format at a time**, registry-driven. Nutanix, HP, HPE, Lenovo (ISG and IDG), Zebra, Dell, Cisco, Datalogic, Epson, Strike, and Trellix parsers currently ship; new vendors/formats are added as a parser class + fixture + registry entry (see "Adding a parser format" in AGENTS.md).
...
- `docs/hpe_bid_xlsx.md` — HPE Bid (XLSX). HPE deal-export workbooks; header anchor `"LineType"` (one word); `Part Number`/`Bundle`/`BundleDetails` rows; vpn from `ProductNumber`/`BundleID`/`ComponentID`, with `#OptionCode` appended for BundleDetails only; msrp from `ListPrcEst`, cost from `Offering`, qty only from `Quantity`; Min Order Qty and Comments remain blank; BundleDetails msrp/cost → `0.0001` sentinel; no quoted total; available templates `No Calculation` and `Uplift`.
...
- `docs/trellix_quote_pdf.md` — Trellix Distributor Quote Report PDF. AUD-only, two dense table layouts, dual quantity selection, unit MSRP calculation, dates/serial/comments, and Total Distribution Cost validation.
- `docs/trellix_quote_xlsm.md` — Trellix Distributor Quote workbook. AUD-only, displayed/underlying date pairs, cached formula values, dual quantity selection, and summed Final Disti Cost validation.
...
`shadow-xs`, `#0077d4` accent, `#0063FF` brand blue, Inter typography, uppercase tracked labels) is
