...
`samples/inputs/99000001.xlsx` — 1 parent + 30 children. Config ID `99000001`, Config Name `HP EliteBook 6 G2i 14 AI`, Total Price `6,042.77 AUD`.
...
Writer: `CrmWriter`. Column mapping:
...
| X | `IM%` | user-supplied `imPercent` (always written) |
...
| `imPercent` | Yes | Written to col X for all rows; persisted as `User.ImPercent` |
| `fxRate` | No | Not used (AUD, no FX conversion) |
| `crmTemplate` | No | Always `% Off RRP with Uplift` — omit or pass the literal string |

## Bid metadata

The validated `Config ID` is `BidNumber`; revision is `1`. The existing `config`-stage errors are unchanged — this format cannot reach the parse without a Config ID.
