...
Complete edge `Page N of M` footers are removed by the shared PDF reconstruction helper before row
grouping and column splitting. The parser retains its row-level footer check as a second safeguard.
...
**Part Number**
From the `Product Code` column. The Part Number itself can wrap across two visual lines (e.g. `NX-1175S-G10-` on one line and `6517P-CM` on the next, or `C-NVM-7.68TB-` and `AB1A-CM`). Concatenate the anchor + continuation snippets with **no separator** (the wrap usually happens mid-token after a hyphen), trim whitespace, then convert the parsed value to uppercase so it matches the SAP SKU identifier. Plain source labels such as `Support-Term` and `Platform Integration` therefore emit as `SUPPORT-TERM` and `PLATFORM INTEGRATION`; `Raw` retains the source casing.
...
Compute the line total by multiplying `Cost` by `Qty` for every kept row and summing. Bundled-component rows contribute `$0 × qty = $0`, the `Support-Term` row contributes `$0 × 60 = $0`, and the `Platform Integration` row contributes `$0 × 1 = $0`. The remaining priced rows must sum to the quoted total. For `XQ-9100003.pdf` Quote D the sum is `22,491.87`, matching the quoted total exactly.

## Bid metadata

After locating Quote D, use the last preceding `Quote: XQ-... - Parent Quote` header — the nearest one above the section actually extracted; revision is `1`. Best-effort — no match leaves both fields null rather than failing the parse.
