...
XQ-9100004.pdf is an example of a Renewal (PDF) quote from which the parser extracts line items.

**Sample File:** XQ-9100004.pdf
...
**Part Number**
Extract the part number from the `Product Code` column, join wrapped fragments without spaces, and convert the parsed value to uppercase so it matches the SAP SKU identifier. `Raw` retains the source casing. In XQ-9100004.pdf, the emitted values are as follows.
...
This value is stored on the line item as a single field `serial_number` and written to `Serial Number` (column M) in the CRM output. We do **not** split it into separate serial/license fields.
...
**MSRP**
Using the "Term Adjusted List Unit Price" column extract the number in each row. The string in this field will look like "USD 77.00" or "USD 575.00". Strip the "USD" prefix and any commas, and parse as a number. For example "USD 77.00" becomes "77" and "USD 575.00" becomes "575". In `XQ-9100004.pdf` the kept values are:
...
**Sale Price (`Cost`)**
Using the `Net Unit Price` column, extract the number in each row. The string in this field will look like `USD 54.41` or `USD 371.83`. Strip the currency text `USD` and capture only the value. For example, `USD 54.41` becomes `54.41` and `USD 371.83` becomes `371.83`.

**Quantity**
Using the "Qty" column, extract the quantity. In the provide sample XQ-9100004.pdf, you would end up with the values;
...
**Validation**
Once all lines are extracted, you will need to validate that the values are captured accurately. To achieve this I would first look to the "TOTAL: " field at the bottom of the table, in the sample file XQ-9100004.pdf this contains the string "USD 60,205.68". You would need to strip the currency text "USD" and parse the raw number which is 60205.68.

With the raw number, calculate the total value using the extracted values for each line. Multiply each line's `Cost` by `Qty`, then add them all. The result should match the quoted total of `60205.68`.

## XQ-9100007.pdf Sample (Wrapped Prices)
...
| No | Product Code | Serial Number | Start Date | End Date | MSRP | Sale Price | Quantity | Total Net Price |
|----|--------------|---------------|------------|----------|------|------------|----------|-----------------|
...
## XQ-9100001.pdf Sample (Platform Column Variant)
